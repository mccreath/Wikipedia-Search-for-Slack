...
### Understanding the returned data, or: A short discursion into JSON and arrays.
...
In an indexed array, we reference each item in the array by its index. Using the array above, if we wanted to make a variable with the title for the Wikipedia page "Airship", we would use `$wiki_array[1][0]`. That basically tells PHP "Look in the array called `$wiki_arr`, go to index `[1]`, and within that, find index `[0]`.
...
The information you send into Slack and how it's formatted will directly effect how successful your integration is. You want to provide enough data for the user to take any action that might need, but not so much that it overwhelms the channel. In the case of our Wikipedia search, the message is something that will be triggered by a human user so there shouldn't be too much concern about spamming a channel, at least from a technical standpoint. Other considerations:

* We want to identify the message as coming from Slackipedia with a custom username and a custom icon. These are both helpful for team members to see in the context of a Slack channel to help distinguish human messages from bot and integration messages.
* We _also_ want to identify the team member that caused the message to be posted. This good etiquette for both the message poster and their fellow team members.
...
* If Wikipedia doesn't have one page that it considers the best for that search, it will return a link to a disambiguation page, along with a list of possible options. In those cases, we need to make sure that's clear to the user.

The username and icon are simple. Slack's incoming webhooks have parameters for both of those. The username is just text. The icon option takes a URL, so you'll need to upload the icon that comes with this package to a publicly-available file server. This can be the server where you're running the script for this webhook, or it can be Dropbox, AWS, or any other file service.

```json 
{
	"username":"Slackipedia",
	"icon_url":"http://yourserver.com/path/to/icon 
}
```

You can also set both of the values on your webhook configuration page. Setting the name on the configuration page can be very useful for finding Slackipedia in your list of integrations if you have a lot of incoming webhooks.

Let's take a look at the options we have for displaying all the data coming through an incoming webhook. The main thing you send in with a webhook is the message itself, which has a few options. The `text` parameter of the message is required _unless_ you send in `attachments`. Attachments in incoming webhooks are not attachments in the sense of email attachments; they're not files. They're additional chunks of data that can help with formatting and clarifying the message that your webhook posts. You can have up to ten attachments per message. 
...
So, we'll use an `if` statement to see if we got something, and if we did, we'll decode the JSON and put it into a PHP array, which is much faster and easier to work with than the JSON text string.

```php
if($wiki_resp !== FALSE){
	$wiki_array = json_decode($wiki_resp);
```

While we're working with the array, let's get our non-primary results into their own array so they'll be easier to work with. Since we're only going to display the link for each of these items, we only need to get the data from the third sub-array in the $wiki_array array. Since we only need the alternate options, we can use `array_shift` to remove the first item from our new array. Finally, we'll set the number of alternate options as a variable that we can display later.
